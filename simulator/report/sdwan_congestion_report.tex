...
\title{SD-WAN Congestion Detection and Anomaly Identification Using Machine Learning Algorithms: A Hospital Network Simulation\\
{\footnotesize Computer Networks and Communications Project Report}
}

\author{\IEEEauthorblockN{Gopal}
...
\begin{IEEEkeywords}
SD-WAN, Congestion Detection, DDoS Attack, Shannon Entropy, EWMA, Token Bucket, Network Simulation, Hospital Network
\end{IEEEkeywords}
...
Software-Defined Wide Area Networks (SD-WAN) have emerged as a solution to these challenges by providing centralized traffic management, quality of service (QoS) enforcement, and enhanced security features \cite{b1}. However, the complexity of modern network threats, particularly DDoS attacks, requires sophisticated detection mechanisms that can distinguish between legitimate congestion and malicious traffic patterns.
...
Our simulation models a hospital network with the following components:
...
\begin{algorithmic}
\STATE $tokens \gets \min(bucket\_capacity, tokens + capacity \times dt)$
\STATE $needed \gets load \times dt$
\IF{$needed > tokens$}
    \STATE $packet\_loss \gets (needed - tokens) / needed$
    \STATE $tokens \gets 0$
\ELSE
    \STATE $tokens \gets tokens - needed$
    \STATE $packet\_loss \gets 0$
\ENDIF
\end{algorithmic}
...
\begin{equation}
CSS = 0.5 \cdot D_f + 0.3 \cdot L_f + 0.2 \cdot E_f
\end{equation}

where:
\begin{itemize}
    \item $D_f$ = Delay Factor = $\min(3.0, RTT_{ewma} / RTT_{base})$
    \item $L_f$ = Loss Factor = $packet\_loss \times 10$
...
The simulation backend is implemented in Python using the following technologies:

\begin{itemize}
    \item \textbf{Flask}: RESTful API server for frontend communication
    \item \textbf{NetworkX}: Graph-based network topology modeling
    \item \textbf{Threading}: Background simulation loop at 20 Hz update rate
\end{itemize}

\begin{lstlisting}[caption={Core Simulation Loop}]
def run_simulation():
    while True:
        if sim.running:
            sim.update()
...
The web-based frontend is built using:

\begin{itemize}
    \item \textbf{React}: Component-based UI framework
    \item \textbf{Vite}: Build tool with hot module replacement
    \item \textbf{SVG}: Scalable vector graphics for network topology
    \item \textbf{Framer Motion}: Animation library for smooth transitions
\end{itemize}

Key visualization features include:
\begin{itemize}
    \item Real-time topology display with draggable nodes
    \item Color-coded congestion levels (Blue < Green < Orange < Red)
    \item Animated packet flow along network links
    \item Terminal-style log viewer for system alerts
\end{itemize}
...
\hline
/api/control & POST & Start/Stop/Reset simulation \\
\hline
/api/trigger & POST & Trigger DDoS attack \\
\hline
\end{tabular}
\label{tab2}
\end{center}
